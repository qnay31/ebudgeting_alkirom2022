...
if ($_GET["id_dataManagement"] == "program") {
    ...
    $q3  = mysqli_query($conn, "SELECT * FROM 2022_program WHERE id_pengurus = '$_SESSION[id_pengurus]' AND `posisi` = '$_SESSION[posisi]' AND laporan = 'Belum Laporan' AND id = '$unik' AND MONTH(tgl_pengajuan) = '$periode' ORDER BY `tgl_pengajuan` DESC");
    $data   = mysqli_fetch_assoc($q3);
        ...
        $q3  = mysqli_query($conn, "SELECT * FROM 2022_logistik WHERE id_pengurus = '$_SESSION[id_pengurus]' AND `posisi` = '$_SESSION[posisi]' AND laporan = 'Belum Laporan' AND id = '$unik' AND MONTH(tgl_pengajuan) = '$periode' ORDER BY `tgl_pengajuan` DESC");
        $data   = mysqli_fetch_assoc($q3);
        
} elseif ($_GET["id_dataManagement"] == "paudqu") {
    if (isset($_POST["input"]) ) {
        $link = $_SESSION["username"];
        if(edit_anggaran_paudqu($_POST) > 0 ) {
                echo "<script>
                        alert('Data anggaran Berhasil Diedit');
                        document.location.href = '$link.php?id_forms=forms_verifikasi&id_dataManagement=$id_management';
                    </script>";
    ...
    $q3  = mysqli_query($conn, "SELECT * FROM 2022_paudqu WHERE id_pengurus = '$_SESSION[id_pengurus]' AND `posisi` = '$_SESSION[posisi]' AND laporan = 'Belum Laporan' AND id = '$unik' AND MONTH(tgl_pengajuan) = '$periode' ORDER BY `tgl_pengajuan` DESC");
    $data   = mysqli_fetch_assoc($q3);
    ...
    if (isset($_POST["input"]) ) {
        ...
        $q3  = mysqli_query($conn, "SELECT * FROM 2022_$id_management WHERE id_pengurus = '$_SESSION[id_pengurus]' AND `posisi` = '$_SESSION[posisi]' AND laporan = 'Belum Laporan' AND id = '$unik' AND MONTH(tgl_dibuat) = '$periode' ORDER BY `tgl_dibuat` DESC");
        $data   = mysqli_fetch_assoc($q3);
}
// die(var_dump($s));
            ...
            <div class="col-lg-12" id="form-pengajuan">
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <?php include '../models/forms/edit_pengajuan/edit_program.php'; ?>

                        <?php } elseif ($_GET["id_dataManagement"] == "logistik") { ?>
                        <?php include '../models/forms/edit_pengajuan/edit_logistik.php'; ?>

                        <?php } else { ?>
                        <?php include '../models/forms/edit_pengajuan/edit_management.php'; ?>
                        <?php } ?>
                    </div>
                </div><!-- End Laporan  -->
