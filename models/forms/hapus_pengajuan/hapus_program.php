...
// die(var_dump($hapus));
$query2 = mysqli_query($conn, "DELETE FROM `2022_program` WHERE id = '$unik' AND deskripsi = '$deskripsi' AND MONTH(tgl_pengajuan) = '$periode' ");
...
if ($_SESSION["id_pengurus"] == "admin_web") {
    ...
    if ($query2 == true) {
        echo "<script>
        alert('Data Berhasil Dihapus');
        document.location.href = '../../../admin/$_SESSION[username].php?id_forms=forms_verifikasi&id_dataManagement=program';
        </script>";
    }  else {
        echo "<script>
        alert('Data Tidak Berhasil Dihapus');
