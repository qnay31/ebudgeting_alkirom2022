...
$unik     = $_GET["id_unik"];
$periode  = $_GET["id_p"]; 
$query  = mysqli_query($conn, "SELECT * FROM 2022_operasional_yayasan WHERE id = '$unik' AND MONTH(tgl_dibuat) = '$periode' ");
...
$update = mysqli_query($conn, "UPDATE `2022_operasional_yayasan` SET 
            `status`  ='OK'
            WHERE id    = '$unik' "); 
...
document.location.href = '../../admin/$_SESSION[username].php?id_checklist=checklist_pengajuanOperasional';
</script>";
